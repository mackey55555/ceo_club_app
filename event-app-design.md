...
flowchart TD
    ...
    NEWS -->|押下| NEWS_DETAIL
    EVENT -->|押下| EVENT_DETAIL
    EVENT_DETAIL -->|申込み| EVENT_APPLY
    HISTORY -->|WebView| EXT["外部サイト"]
...
| 6 | イベントスケジュール | `/events` | イベントリスト（年月フィルター） |
| 7 | イベント詳細 | `/events/[id]` | イベント本文・申込みフォーム |
| 8 | 参加履歴 | `/history` | WebView表示 |
| 9 | 会員証 | `/member` | プロフィール表示 |
| 10 | プロフィール編集 | `/member/edit` | プロフィール編集フォーム |
